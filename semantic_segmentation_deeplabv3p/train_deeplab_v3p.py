...
from dataset_utils import WoodScapeDataset
from dataset_utils import WoodScapeDataModule
from dataset_utils import Cityscapes
from dataset_utils import CityscapesDataModule
...
def main(args):
    ...
    #      Transfer Learning     #
    ##############################
    # ToDo!: Fix this condition block: Loss Function is also using require_grad = False!
    # Feature Extraction Mode: Train Only specific layers!

    # if not fine_tuning:  # Update only the layer we added/changed/introduced
    #     # Freeze all layers
    #     for param in model_module.deeplab_v3plus_model.parameters():
    #         param.requires_grad = False
    #     # Unfreeze only the layers that we care
    #     for name, param in model_module.deeplab_v3plus_model.named_parameters():
    #         splitted_name = name.split('.')
    #
    #         if splitted_name[0] == "classifier" and splitted_name[1] == "vi_bbb_conv2d":
    #             ic(name)
    #             param.requires_grad = True
    # # Fine Tuning Mode: load a pre-trained model and update all the model's parameters
    # else:
    #     ic("updating all the pre-loaded model parameters")
    #     # In fine_tuning we start with a pre-trained model and update all the model parameters
