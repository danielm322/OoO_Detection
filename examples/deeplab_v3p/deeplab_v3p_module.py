...
class DeepLabV3PlusModule(pl.LightningModule):
    def __init__(
        ...
        self.deeplab_v3plus_model = deeplab_v3plus(
            self.backbone_name,
        ...
        self.metric_accuracy = torchmetrics.Accuracy(
            num_classes=self.n_class, mdmc_average="samplewise"
        )
    ...
    def get_criterion(self):
        if self.deeplabv3plus_type == "normal" or self.deeplabv3plus_type == "backbone_dropblock2d":
            ...
            if self.pred_loss_type == "focal_loss":
                criterion = ELBOWeightVILoss(
                    pred_loss_type="focal_loss", img_pred_weight=self.img_pred_weight
                )
            elif self.pred_loss_type == "cross_entropy":
                criterion = ELBOWeightVILoss(
                    pred_loss_type="cross_entropy", img_pred_weight=self.img_pred_weight
                )
            else:
                raise ValueError("Not a valid loss type!")
        else:
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.SGD(
            self.deeplab_v3plus_model.parameters(),
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        if self.deeplabv3plus_type == "variational_layer":
            ...
            self.log_dict(
                {"train_loss": train_loss, "train_accuracy": accuracy, "train_IoU": jaccard_idx},
                on_step=False,
                on_epoch=True,
                prog_bar=True,
            )
    ...
    def on_train_epoch_end(self) -> None:
        if self.label_colours is not None:
            ...
            with torch.no_grad():
                for i, file in enumerate(img_folder):
                    ...
                    prediction = torch.FloatTensor(
                        [self.label_colours[p.item()] for p in pred.view(-1)]
                    ).to(self.device)
                    prediction = prediction.transpose(0, 1).view(3, 483, 640) / 255.0
            ...
            self.logger.experiment.add_image(
                "pred_test_images", grid_log, global_step=self.global_step
            )
