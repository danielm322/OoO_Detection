...
# from loss import VIELBOWeightLoss
from .loss import calculate_kl as KL_DIV
...
class BBBConv2d(ModuleWrapper):
    def __init__(
        ...
        if priors is None:
            priors = {
                ...
                "posterior_rho_initial": (
                    -2,
                    0.1,
                ),  # if value is lower than -3, it can cause instabilities
            }
        ...
        self.W_mu = Parameter(
            torch.empty((out_channels, in_channels, *self.kernel_size), device=self.device)
        )
...
# class BBBConv2dRepo(ModuleWrapper):
