...
def anomaly_mud(
    ...
    anomaly_width=2048,
    anomaly_height=1024,
    dirt_mud_images_path="./dataset_utils/dirt_mud_images/",
):
...
class AnomalyMud(ImageOnlyTransform):
    def __init__(
        ...
        anomaly_height=1024,
        p=0.5,
        dirt_mud_imgs_path="./dataset_utils/dirt_mud_images/",
    ):
        super().__init__(p=p)
    ...
    def apply(self, img_cv, **params):
        ...
        return anomaly_mud(
            pil_img, self.anomaly_width, self.anomaly_height, self.dirt_mud_imgs_path
        )


# Synthetic anomalies Unit-Test
def main():
    # test_img_path = "./dataset_utils/test_images/cityscapes_test_img.png"
    ...
    synthetic_anomaly_transforms = A.Compose(
        [
            ...
            A.OneOf(
                [
                    AnomalyMud(anomaly_width=512, anomaly_height=256, p=1),
                    A.RandomFog(fog_coef_lower=0.7, fog_coef_upper=0.8, alpha_coef=0.3, p=1),
                    A.RandomSunFlare(
                        flare_roi=(0.3, 0.1, 0.7, 0.9),
                        src_radius=250,
